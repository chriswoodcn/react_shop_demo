...
export default class RegIndex extends React.Component {
    ...
    //点击注册按钮提交数据
    async submitData() {
        if (isEmpty(this.state.sCellphone)) {
        ...
        //     return false;
        // }
        if (!isMobilePhone(this.state.sCellphone, 'zh-CN')) {
    ...
    render() {
        return (
            <div className={Css['page']}>
                <SubHeaderComponent title="会员注册"/>
                <div className={Css['main']}>
                    <div className={Css['cellphone-wrap']}>
                        <div className={Css['cellphone']}><input type="tel" placeholder="请输入手机号" onChange={(e) => {
                            this.checkCellphone(e)
                        }}/></div>
                        <div
                            className={this.state.bCodeSuccess ? Css['code-btn'] + " " + Css['success'] : Css['code-btn']}
                            onClick={this.getCode.bind(this)}>{this.state.sCodeText}</div>
                    </div>
                    <div className={Css['code-wrap']}><input type="text" placeholder="请输入短信验证码" onChange={(e) => {
                        this.setState({sCode: e.target.value})
                    }}/></div>
                    <div className={Css['password-wrap']}>
                        <div className={Css['password']}><input type={this.state.sType} placeholder="请输入密码"
                                                                onChange={(e) => {
                                                                    this.setState({sPassword: e.target.value})
                                                                }}/></div>
                        <div className={Css['switch-wrap']}>
                            <Switch color="#EB1625" checked={this.state.checked}
