...
import {getGoodsInfo, getGoodsAttr, getReviews, addFav} from "../../../api";
...
class DetailsItem extends React.Component {
    ...
    //获取商品轮番图和获取商品信息
    setGoodsInfo() {
        getGoodsInfo(this.state.gid).then((res) => {
            if (res.code === 200) {
                this.setState({
    ...
    //获取商品规格属性
    setGoodsAttr() {
        getGoodsAttr(this.state.gid).then(res => {
            if (res.code === 200) {
    ...
    addFav() {
        if (this.props.state.user.isLogin) {
            addFav(this.props.state.user.uid, this.state.gid).then(res => {
    ...
    addCart() {
        this.checkAttrVal(() => {
            if (!this.bMove) {
                this.bMove = true;
                let oGoodsImg = this.refs['goods-img'], oGoodsInfo = this.refs['goods-info'],
                ...
                oCloneImg.style.cssText = "border-radius:100%;over-flow:hidden;width:0.4rem;height:0.4rem;position:absolute;z-index:1;left:0.2rem;top:0.2rem;";
                ...
                TweenMax.to(oCloneImg, 2, {
                    bezier: [{x: srcImgX, y: -100}, {x: srcImgX + 30, y: -130}, {x: oCartIcon.offsetLeft, y: -cloneY}],
                    onComplete: () => {
                        oCloneImg.remove();
                        this.bMove = false;
        ...
        return (
            <div>
                    ...
                    <div className="swiper-wrapper">
                        {
                            this.state.aSlide.length > 0 ?
                                this.state.aSlide.map((item, index) => {
                                    return (
                                        <div className="swiper-slide" key={index}><img src={item}
                                                                                       alt={this.state.sGoodsTitle}/>
                                        </div>
                                    )
                                })
                                : ''
                ...
                <div className={Css['reviews-main']}>
                    ...
                    <div className={this.state.iReviewTotal > 0 ? Css['reviews-more'] : Css['reviews-more'] + " hide"}
                         onClick={this.replacePage.bind(this, 'goods/details/reviews?gid=' + this.state.gid)}>查看更多评价
                    </div>
                </div>
                ...
                <div ref="cart-panel" className={Css['cart-panel'] + " " + this.state.sCartPanel}>
                    <div ref="goods-info" className={Css['goods-info']}>
                        <div className={Css['close-panel-wrap']}>
                            ...
                            <div className={Css['close']} onClick={this.hideCartPanel.bind(this)}/>
                        </div>
                        <div ref="goods-img" className={Css['goods-img']}>
                            <img src={this.state.aSlide.length !== 0 ? this.state.aSlide[0] : ''}
                                 alt={this.state.sGoodsTitle}/>
                        </div>
                    ...
                    <div className={Css['amount-wrap']}>
                        ...
                        <div className={Css["amount-input-wrap"]}>
                            <div
                                className={this.state.iAmount <= 1 ? Css['btn'] + " " + Css['dec'] + " " + Css["active"] : Css['btn'] + " " + Css['dec']}
