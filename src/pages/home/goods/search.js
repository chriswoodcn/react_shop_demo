...
import {getPageData, getScrollPage, getClassifyData, getAttrData} from "../../../api";
...
export default class GoodsSearch extends React.Component {
    ...
    setPageData() {
        this.setParams();
        getPageData(this.sParams).then(res => {
            if (res.code === 200) {
    ...
    //获取属性数据
    setAttrData() {
        getAttrData(this.sKeywords).then(res => {
            if (res.code === 200) {
                let aAttr = res.data;
    ...
    getScrollPage() {
        this.oUpRefresh = new UpRefresh({
            ...
            getScrollPage(this.sParams, curPage).then(res => {
                if (res.code === 200) {
            ...
            <div className={Css['page']}>
                ...
                <div className={Css['search-top']}>
                    <div className={Css['search-header']}>
                        <div className={Css['back']} onClick={this.goBack.bind(this)}/>
                        <div className={Css['search-wrap']}>
                            <div className={Css['search-icon']}/>
                            <div className={Css['search-text']}
                                 onClick={this.changeSearch.bind(this)}>{this.state.sKeywords}</div>
                        </div>
                        <div className={Css['screen-btn']} onClick={this.showScreen.bind(this)}>筛选</div>
                    </div>
                    <div className={Css['order-main']}>
                        <div
                            className={this.state.bPriceMenu ? Css['order-item'] + " " + Css['active'] : Css['order-item']}
                            ...
                            <ul className={this.state.bPriceMenu ? Css['order-menu'] : Css['order-menu'] + " hide"}>
                                {
                                    this.state.aPriceOrder.map((item, index) => {
                                        return (
                                            <li key={index} onClick={this.checkedPriceOrder.bind(this, index)}
                                                className={item.checked ? Css['active'] : ''}>{item.title}</li>
                                        )
                                    })
                                }
                            </ul>
                        ...
                        <div
                            className={this.state.bSalesMenu ? Css['order-item'] + " " + Css['active'] : Css['order-item']}
                            ...
                            this.state.aGoods.map((item, index) => {
                                return (
                                    <div key={index} className={Css['goods-list']}
                                        ...
                                        <div className={Css['image']}><img data-echo={item.image}
                    ...
                    <div className={Css['scroll-wrap']}>
                        <div className={Css['attr-wrap']}>
                            <div className={Css['attr-title-wrap']} onClick={this.handleClassify.bind(this)}>
                                ...
                                <div
                                    className={this.state.aClassify.checked ? Css['attr-icon'] : Css['attr-icon'] + " " + Css['up']}/>
                            </div>
                            <div
                                className={this.state.aClassify.checked ? Css['item-wrap'] : Css['item-wrap'] + " height-hide"}>
                                {
                        ...
                        <div style={{width: "100%", height: "1px", backgroundColor: "#EFEFEF"}}/>
                        <div className={Css['attr-wrap']}>
                            <div className={Css['attr-title-wrap']} onClick={this.handlePrice.bind(this)}>
                                ...
                                <div className={Css['price-wrap']}>
                                    <div className={Css['price-input']} onClick={this.preventBubble.bind(this)}><input
                                        type="tel" placeholder="最低价"
                                        value={this.state.fPrice1 === 0 ? '' : this.state.fPrice1}
                                        onChange={this.changePrice1.bind(this)}/></div>
                                    <div className={Css['price-line']}/>
                                    <div className={Css['price-input']} onClick={this.preventBubble.bind(this)}><input
                                        type="tel" placeholder="最高价"
                                        value={this.state.fPrice2 === 0 ? '' : this.state.fPrice2}
                                        onChange={this.changePrice2.bind(this)}/></div>
                                </div>
                                <div className={Css['attr-icon']}/>
                            </div>
                            <div
                                className={this.state.aPrice.checked ? Css['item-wrap'] : Css['item-wrap'] + " height-hide"}>
                                {
                                        ...
                                        <React.Fragment key={index}>
                                            <div className={Css['attr-wrap']}>
                                                <div className={Css['attr-title-wrap']}
                                                     onClick={this.handleAttr.bind(this, index)}>
                                                    <div className={Css['attr-name']}>{item.title}</div>
                                                    <div
                                                        className={item.checked ? Css['attr-icon'] : Css['attr-icon'] + " " + Css['up']}/>
                                                </div>
                                                <div
                                                    className={item.checked ? Css['item-wrap'] : Css['item-wrap'] + " height-hide"}>
                                                    {
                                                        item.param.length > 0 ?
                ...
                <SearchComponent pageStyle={this.state.pageStyle} childStyle={this.getStyle.bind(this)} isLocal="1"
                                 childKeywords={this.getChildKeywords.bind(this)} keywords={this.state.sKeywords}/>
            </div>
        );
    }
}
